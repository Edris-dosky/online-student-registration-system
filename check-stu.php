...
<?php
    ...
    if ($part!="کۆلێژی سۆران"){
        $call=mysqli_query($db,"SELECT * FROM `form`  WHERE `part`='$part' and `suc`='success' ");
        $xcall=mysqli_query($db,"SELECT * FROM `login`  WHERE `part`='$part' ");
        }else{
            $call=mysqli_query($db,"SELECT * FROM `form` where `suc`='success' ");
...
<?php // پەرەی ئاو قوتابیانەی زانیاریان ناردوە وە لە لایەن ئادمینەوە چیک کراون
