...
<?php  // پەرەی ئەو قوتابیانەی وەرگیراون
    ...
    $xcall=mysqli_query($db,"SELECT `passcode`,`code`,`send`,`type`,`part`,`name`,`id`  FROM `login`  WHERE `part`='$part' and `parallel`='1' ");
    }else{
        $call=mysqli_query($db,"SELECT `blood`,`tell`,`place`,`type`,`part`,`name` FROM `form` where `parallel`='1' ");
        $xcall=mysqli_query($db,"SELECT `passcode`,`code`,`send`,`type`,`part`,`name`,`id`  FROM `login` where `parallel`='1'  ");
    }
